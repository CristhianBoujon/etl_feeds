...
def clear_file(file_name):
    for line in fileinput.FileInput(file_name, inplace = True):
        ...
        clean_line = invalid_entities_remove(clean_line)
        sys.stdout.write(clean_line)


def invalid_entities_remove(text, char = ''):
    entity_regex = re.compile("&(?!(?:quot|amp|apos|lt|gt);)[a-zA-Z0-9]*;")
    return entity_regex.sub(char, text)

def invalid_xml_remove(text, char = ''):
    ...
    return illegal_xml_re.sub(char, text)
